 ...
 */
public class IvyResolver extends AbstractResolver {
    ...
    private static org.fedoraproject.xmvn.artifact.Artifact ivy2aether(
            ModuleRevisionId revision, String extension) {
    ...
    static org.fedoraproject.xmvn.artifact.Artifact ivy2aether(
        ...
        if (classifier == null || classifier.isEmpty()) {
            if (extension.equals("jar") && type.equals("source")) classifier = "sources";
    ...
    private String resolveModuleVersion(ModuleDescriptor module) {
        for (Artifact artifact : module.getAllArtifacts()) {
            ...
            if (result.getArtifactPath() != null) return resolvedVersion(result);
        }
        ...
        if (pomPath != null) {
            ...
            module =
                    DefaultModuleDescriptor.newDefaultInstance(
                            depId, depDescriptor.getAllDependencyArtifacts());
    ...
    public ResolvedModuleRevision getDependency(DependencyDescriptor systemDd, ResolveData data)
        ...
        try {
            ...
            MetadataArtifactDownloadReport report =
                    new MetadataArtifactDownloadReport(module.getMetadataArtifact());
    ...
    public ResolvedResource findIvyFileRef(DependencyDescriptor depDescriptor, ResolveData data) {
        Artifact artifact =
                DefaultArtifact.newIvyArtifact(depDescriptor.getDependencyRevisionId(), null);

        ResolutionRequest request = new ResolutionRequest();
        request.setArtifact(ivy2aether(artifact.getModuleRevisionId(), "pom"));
        ResolutionResult result = getResolver().resolve(request);
        Path pomPath = result.getArtifactPath();
        if (pomPath == null) return null;
    ...
    public DownloadReport download(Artifact[] artifacts, DownloadOptions options) {
        ...
        for (Artifact artifact : artifacts) {
            ...
            if (artifactPath != null) {
                artifactReport.setArtifactOrigin(
                        new ArtifactOrigin(artifact, false, artifactPath.toString()));
    ...
    private void deployEffectivePom(ModuleRevisionId moduleRevisionId, Path artifactPath)
        ...
        try {
            File pomFile =
                    artifactPath
                            .resolveSibling(
                                    artifactPath.getName(artifactPath.getNameCount() - 1)
                                            + "-xmvn.pom")
                            .toFile();
            ModuleDescriptorParser parser = XmlModuleDescriptorParser.getInstance();
            ModuleDescriptor module =
                    parser.parseDescriptor(
                            getSettings(), artifactPath.toFile().toURI().toURL(), false);
